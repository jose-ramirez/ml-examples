...
export default class PCA extends Vue {
  ...
  initialDraw(){
      let d = [
        ["X", "gray"],
        ...iris.map(r => [r.sepal_length, r.sepal_width]),
      ]
  ...
  updateDraw(){
      let d = [
        ...
        ...this.model.parameters.projected_data.slice(0, 50).map(y => [y[0], y[1], null, null]),
        ...this.model.parameters.projected_data.slice(50, 100).map(b => [b[0], null, b[1], null]),
        ...this.model.parameters.projected_data.slice(100, 150).map(g => [g[0], null, null, g[1]])
      ]
      let dataPoints = GoogleCharts.api.visualization.arrayToDataTable(d)
      let chart = new GoogleCharts.api.visualization.ScatterChart(document.getElementById("chart_div"))
